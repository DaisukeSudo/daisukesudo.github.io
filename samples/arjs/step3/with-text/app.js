...
const renderPlace = ({ label, location }, count) => {
  ...
  const textId = `t-${Math.floor(Math.random() * 100000)}`;
  const $text = createText({ id: textId, location, value: label });
  ...
  const buttonClickEventListener = () => {
    ...
    $scene.removeChild(document.getElementById(textId));
    renderPlace({ label, location }, count + 1);
  }
